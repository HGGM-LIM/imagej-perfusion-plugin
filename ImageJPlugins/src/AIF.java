...
public class AIF implements ItemListener ,WindowListener {
	...
	public AIF(List<VoxelT2> AllVoxels,double max)   {
		//EllipseRoi er = new EllipseRoi(10,10,20,20,10);
		//voxelsROI(er);
	...
	public List<VoxelT2> voxelsROI (List<VoxelT2> allV ) {
		...
		for (int j =0; fromIn ==-1 && j < points[0].length; j++)
		 fromIn = allV.indexOf(VoxelT2.VoxelSearch(allV, points[0][j], points[1][j], z2));
		
		List<VoxelT2> subList = allV.subList(fromIn, allV.size() -1);
		for(int j = points[0].length -1; toIn ==-1 && j > 0; j-- )
		 toIn = subList.indexOf(VoxelT2.VoxelSearch(subList, points[0][j], points[1][j], z2));
		////////////////////////////////////////////
		
		 subList = subList.subList(0, toIn);
		for(int j =0;j < points[0].length; j++ /*Point p : points*/){
	
		VoxelT2 v =VoxelT2.VoxelSearch(subList, points[0][j], points[1][j], z2);
		if (v != null)
			res.add(v);
		}
		
		} else {
			VoxelT2 v =VoxelT2.VoxelSearch(allV, points[0][0], points[1][0], z2);
			if (v != null )
			res.add(v);
		}
		}
	...
	public static int[][] getPointsROI(Roi roi) {
		...
		for (int j=0; j<mask.length; j++){ 
		    if (mask[j] != 0) n++;  //count points in mask 
		} 
		...
		//int i = 0;
		/* By the type is possible to know the Roi's shape
		 type 10 corresponds with a point roi*/
