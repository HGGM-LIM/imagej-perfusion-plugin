...
public class Dynamic_Pixel_Inspector implements PlugInFilter, ActionListener,
	...
	public void run(ImageProcessor arg0) {
	    ...
	    enabled.setFont(new Font("Arial", Font.BOLD,12));
	    changeEnabledLabel();
		jf = createFrame();		
	...
	public int setup(String arg0, ImagePlus imp) {
	    ...
	    if(!imp.isHyperStack()) {
    		...
    		this.imp = imp;		
    		//dim[0] = width; dim[1] = height; dim[2] = channels; dim[3] = slices; dim[4] = frames
    ...
    public void mouseClicked(MouseEvent e) {
        ...
        double[] y = getTAC(offscreenX, offscreenY,imp.getChannel(), imp.getSlice());
     ...
     * coordinates.
     */
    private double[] getTAC(int x, int y,int channel, int slice) {
        
        // Dimension check
        if (x >= dim[0] || x < 0 || y >= dim[1] || y < 0 || slice > dim[3]
                || slice < 1 || channel > dim[2] || channel < 1) {
            return null;
        }
        ...
        for (int frame = 1; frame <= dim[4]; frame++) {
            int stack_number = imp.getStackIndex(channel, slice, frame);            
